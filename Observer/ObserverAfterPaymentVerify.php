...
namespace Itex\ItexPay\Observer;
...
class ObserverAfterPaymentVerify implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $order = $observer->getItexPayOrder();
        
        if ($order && $order->getStatus() == "pending") {
            // sets the status to processing since payment has been received
            $order->setState(Order::STATE_PROCESSING)
                    ->addStatusToHistory(Order::STATE_PROCESSING, __("ItexPay Payment Verified and Order is being processed"), true)
